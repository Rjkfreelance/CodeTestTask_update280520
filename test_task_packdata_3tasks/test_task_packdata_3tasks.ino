...
void task_Packdata_WriteRead_Ram(void* Param){
  ...
  while(1){
    Serial.println("Task PackData & WriteRam Run every 1 sec");
  v_fw[0] = 0x30;
  v_fw[1] = 0x31;
  v_fw[2] = 0x31;
  v_fw[3] = 0x30;
...
void task_WriteRamSDcard(void* Param){
  
  for(;;){
    Serial.println("Task Run every 5 sec  Pre write to SD card wait for write_addeeprom >= 32000");
...
void task_LedStatus(void* Param){
  for(;;){
...
void setup() {
  ...
  delay(100);
    Serial.println("Version: Test Task ,PackDataWriteRam(Core1) ,WriteSDcard(Core1) LedStatus(Core0)");
